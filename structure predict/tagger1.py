...
def tri_decode(words, dictionary, model):

	def backtrack(i, tag, prev_1):
		if i == 1:
			return []
		return backtrack(i-1, prev_1, back[i][tag][prev_1]) + [tag]
	...
	for i, word in enumerate(words[2:], 2):
		for tag in dictionary[word]:
			for prev_1 in best[i-1]:
				for prev_2 in best[i-1][prev_1]:
					score = best[i-1][prev_1][prev_2] + model[prev_2,prev_1,tag] + model[tag, word] + model[prev_1, tag] + model[tag]
					if score > best[i][tag][prev_1]:
						best[i][tag][prev_1] = score
						back[i][tag][prev_1] = prev_2
		#print i, word, dictionary[word], best[i]
	#print best[len(words)-1][stopsym]
...
def test(filename, dictionary, model):	  
	...
	errors = tot = 0
	for words, tags in readfile(filename):
		mytags = tri_decode(words, dictionary ,model)
