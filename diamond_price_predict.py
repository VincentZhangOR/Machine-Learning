...
def train(XY,C,epoch,index2feature):
	...
	for name in classifier_models:
		...
		# code for calculating feature importance
		'''
		classifier_models[name].fit(X, Y)
		w = classifier_models[name].feature_importances_
		sort_w = sorted(zip(w, range(len(w))))
		print 'most negative features:', sort_w[:5]
		for w, index in sort_w[:5]:
			print index2feature[index]
		print 'most positive features:', sort_w[-5:]
		for w, index in sort_w[-5:]:
			print index2feature[index]
		'''
...
def regression(XY, epoch, index2feature, C):
	...
	for name in regressor_models:
		...
		# code for calculating feature importance
		'''
		regressor_models[name].fit(X, Y)
		w = regressor_models[name].feature_importances_
		sort_w = sorted(zip(w, range(len(w))))
		print 'most negative features:', sort_w[:5]
		for w, index in sort_w[:5]:
			print index2feature[index]
